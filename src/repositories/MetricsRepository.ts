...
import IProjectMetricsRepository from './repositoryInterface/IProjectMetricsRepository';
  ...
  public async getMetricsFromProjectMetrics(useCache: boolean = true, context?: WebPartContext, selectedMetrics?: string, selectedProjectType?: string): Promise<IMetrics[]> {
    ...
    try {
      ...
      const items = await this.service.fetchAllItems<any>({
        ...
        //select: selectFields,
        pageSize: 2000,
        filter: 'IsActive eq 1 and VersionId eq ' + (this.VersionId) + ' and ProjectType eq (\'' + (selectedProjectType) + '\')',
